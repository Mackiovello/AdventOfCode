...
mod tests {
    ...
    fn finds_non_overlapping_claim() {
        // given
        let claim_descriptions = vec!["#1 @ 1,3: 4x4", "#2 @ 3,1: 4x4", "#3 @ 5,5: 2x2"];
        let claims = parse_claims(claim_descriptions);

        // that
        let non_overlapping_id = find_non_overlapping_claim(claims);

        // then
        assert_eq!(3, non_overlapping_id);
    }
